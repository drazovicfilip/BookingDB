...
@app.route('/events', methods=['POST', 'GET', 'FILTER'])
def list_events():
    if( 'logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))
    ...
    if (request.method == 'GET'):
        ...
        try:
            ...
            print(request.form['max'])
            filtered_events = database.all_events_filter(request.form['max'], request.form['page'])
            if(filtered_events is None):
                val = []
                flash("Error, sport \'{}\' does not exist".format(request.form['max']))
                page['bar'] = False

            return render_template('event_list.html', events=filtered_events, session=session)

    elif(request.method == 'FILTER'):
        print(request.form['max'])
        filtered_events = database.all_events_filter(request.form['max'])
        if(filtered_events is None):
            val = []
            flash("Error, sport \'{}\' does not exist".format(request.form['max']))
            page['bar'] = False
